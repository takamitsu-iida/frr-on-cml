...
Ubuntuを起動して、FRRをインストールして、それをCLONEすれば単手番でラボを作れるだろう、と思っていましたが実はそうではありません。
CMLに含まれるUbuntuのイメージはRead Onlyになっていますので、どれだけ変更しても元のイメージが変わることはありません。
そしてクローンできるのは元のイメージの方です。
したがってFRRをインストールしたUbuntuを複数起動したければ、その都度FRRをインストールしなければいけません。
...
UbuntuのSETTINGSタブの Image Definition のドロップダウンから、上記で作成したlabelのものを選んでから起動します。
...
この時点では、Ubuntuを起動するのに使ったイメージはまだ変更されていません。
...
以上のように、FRRのインストールはかなり大変です。
試行錯誤しながら繰り返し実行すると尚更です。
FRRのインストール作業を自動化するansibleのプレイブックを作成しました。
...
このプレイブックの最後では `/var/lib/cloud/` を削除しています。

再起動するとcloud-initが走ってしまいますので、再起動後に手作業でいじった場合は `rm -rf /var/lib/cloud` を忘れずに実行します。
