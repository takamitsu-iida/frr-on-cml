...
疑問なのは、どうやって隣接ルータのMACアドレスを知るのか。
キャプチャしてみた。

ルーティングテーブルのnext hopは隣接ルータのループバックアドレスになっていて、そのループバックアドレスに関してARP解決している。

OpenFabric（というかISIS）ではどの物理インタフェースの先にそのアドレスがいるのかわかっているので、こういうARPもできるのかな。
